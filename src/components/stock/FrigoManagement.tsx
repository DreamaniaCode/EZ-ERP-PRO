...
export const FrigoManagement: React.FC<FrigoManagementProps> = ({ onEditFrigo, onNewFrigo }) => {
  ...
  return (
    <div className="space-y-6" id="frigo-management-page">
      ...
      {/* Detailed Stock View Modal per Frigo */}
      {selectedFrigoDetail && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg border border-gray-300 shadow-2xl max-w-2xl w-full overflow-hidden flex flex-col max-h-[85vh]">
            <div className="px-5 py-4 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
              <div>
                <span className="text-[10px] font-mono font-bold text-blue-700 px-2 py-0.5 bg-blue-50 border border-blue-200 rounded">
                  {selectedFrigoDetail.code}
                </span>
                <h3 className="font-bold text-base text-gray-900 mt-1">
                  Détail du Stock: {selectedFrigoDetail.name}
                </h3>
              </div>
              <button 
                ...
                if (frigoStocks.length === 0) {
                  return (
                    <div className="p-8 text-center text-gray-500 border border-dashed border-gray-300 rounded">
                      ...
                      <p className="font-medium text-xs">Aucun produit actuellement stocké dans ce frigo.</p>
                    </div>
                  );
                }
                  ...
                  <table className="w-full text-left border-collapse border border-gray-200 text-xs">
                    <thead>
                      <tr className="bg-gray-100 border-b border-gray-200 text-gray-700">
                        ...
                        <th className="p-2 border-r border-gray-200">Produit</th>
                        <th className="p-2 border-r border-gray-200 text-right">Quantité (Kg)</th>
                    ...
                    <tbody>
                      {frigoStocks.map(stk => {
                        ...
                        return (
                          <tr key={stk.productId} className="border-b border-gray-200 hover:bg-gray-50">
                            <td className="p-2 font-mono font-bold text-[#0f62fe] border-r border-gray-200">
                              {prd?.code || 'PRD'}
                            </td>
                            <td className="p-2 border-r border-gray-200 font-semibold text-gray-900">
                              {prd?.name || 'Produit Inconnu'}
                            </td>
                            <td className="p-2 border-r border-gray-200 text-right font-mono font-bold text-gray-800">
                              {stk.quantityKg.toLocaleString()} Kg
                            </td>
                            <td className="p-2 text-right font-mono font-bold text-purple-700">
                              {stk.quantityPallets} Pal.
                            </td>
                          </tr>
                        );
                      })}
            ...
            <div className="p-3 bg-gray-50 border-t border-gray-200 flex justify-end">
              <button
                onClick={() => setSelectedFrigoDetail(null)}
                className="px-4 py-1.5 bg-gray-800 text-white rounded text-xs font-semibold hover:bg-black"
              >
                Fermer
              </button>
            </div>
